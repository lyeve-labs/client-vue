...
import { CmsPlugin, useQuery, useMutation, type VueCmsConfig } from '../src/index.js';
...
describe('CmsPlugin', () => {
	it('installs without error', () => {
		const app = createApp({ template: '<div></div>' });
		expect(() => app.use(CmsPlugin, { baseUrl: 'http://localhost:3001' })).not.toThrow();
	});
...
describe('useQuery', () => {
	...
	it('throws error when CmsPlugin is not installed', () => {
		expect(() => useQuery(c => c.get('/test')))
			.toThrow('CmsPlugin must be installed via app.use(CmsPlugin, config)');
	});
});
